...
def get_config():
    parser = argparse.ArgumentParser(description="Kronos-mini Distributed Pretraining")

    # Paths
    parser.add_argument("--data_dir", type=str, default=os.path.join(ROOT_DIR, "data", "pretrain"))
    parser.add_argument("--save_dir", type=str, default=os.path.join(ROOT_DIR, "pretrained_mini_scaled"))
    ...
    parser.add_argument("--tok_batch_size", type=int, default=64, help="Per-GPU batch size")
    parser.add_argument("--tok_lr", type=float, default=2e-4)
    parser.add_argument("--tok_grad_accum", type=int, default=1)

    # Predictor training
    parser.add_argument("--pred_epochs", type=int, default=5)
    parser.add_argument("--pred_batch_size", type=int, default=64, help="Per-GPU batch size")
    parser.add_argument("--pred_lr", type=float, default=4e-4)
...
# ═══════════════════════════════════════════════════════════════════════
# Model Architecture Constants (Kronos-mini official)
# ═══════════════════════════════════════════════════════════════════════
...
def train_tokenizer(args, device, rank, local_rank, world_size, wandb_run):
    ...
    tokenizer = KronosTokenizer(**TOK_ARCH).to(device)
    ...
    print_rank0(f"  Batches/epoch: {len(train_loader):,}")
        ...
        for batch_idx, (batch_x, _) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % args.tok_grad_accum == 0:
                ...
                if is_main_process() and global_step % args.log_interval == 0:
                    ...
                    print(f"    [Epoch {epoch+1}/{args.tok_epochs}, Step {global_step:,}] "
...
def train_predictor(args, device, rank, local_rank, world_size, wandb_run, tokenizer_path, step_offset):
    ...
    # Predictor
    model = Kronos(**PRED_ARCH).to(device)
    ...
    for epoch in range(args.pred_epochs):
        ...
        for batch_idx, (batch_x, batch_stamp) in enumerate(train_loader):
            ...
            if (batch_idx + 1) % args.pred_grad_accum == 0:
                ...
                if is_main_process() and global_step % args.log_interval == 0:
                    lr = optimizer.param_groups[0]['lr']
                    print(f"    [Epoch {epoch+1}/{args.pred_epochs}, Step {global_step:,}] "
...
def main():
    ...
    print_rank0("╔══════════════════════════════════════════════════════════════╗")
    print_rank0("║    Kronos-mini Distributed Pretraining                      ║")
    print_rank0("╚══════════════════════════════════════════════════════════════╝")
    ...
    if is_main_process() and not args.no_wandb and wandb is not None:
        run_name = args.wandb_run_name or f"kronos-mini-pretrain-{world_size}gpu"
        config_dict = {
            "tokenizer_arch": TOK_ARCH,
            "predictor_arch": PRED_ARCH,
            "training": vars(args),
